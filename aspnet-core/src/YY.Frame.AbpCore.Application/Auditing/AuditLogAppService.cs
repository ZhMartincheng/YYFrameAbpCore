...
using YY.Frame.AbpCore.Auditing.Exporting;
		...
		public List<NameValueDto> GetEntityHistoryObjectTypes()
		{
			...
			if (AbpSession.TenantId == null)
			{
				entityHistoryConfig = entityHistoryConfig
					.Where(c => YY.Frame.AbpCore.EntityHistory.EntityHistoryHelper.HostSideTrackedTypes.Contains(c.Value))
					.ToDictionary(key => key.Key, value => value.Value);
			}
			else
			{
				entityHistoryConfig = entityHistoryConfig
					.Where(c => YY.Frame.AbpCore.EntityHistory.EntityHistoryHelper.TenantSideTrackedTypes.Contains(c.Value))
					.ToDictionary(key => key.Key, value => value.Value);
			}
